...
def parse_email_data(text_content: str) -> dict:
    ...
    location_match = re.search(
        r"Hvor er barnet i hverdagen:\s*(.+?)(?:\n|$)", text_content, re.IGNORECASE
    )
    if location_match:
...
class MailService:
    ...
    async def _initialize_graph_client(self) -> None:
        ...
        try:
            ...
            credential_data = (
                self.credential_obj.data if hasattr(self.credential_obj, "data") else {}
            )
            ...
            scope = credential_data.get(
                "graph_scope", "https://graph.microsoft.com/.default"
            )
            self.graph_client = GraphServiceClient(
                credentials=self.credential, scopes=[scope]
            )
    ...
    async def _test_authentication(self) -> None:
        ...
        try:
            ...
            raise Exception(f"Failed to authenticate with Microsoft Graph: {e}") from e
    ...
    def _extract_message_info(self, msg) -> Dict[str, Any]:
        ...
        return {
            ...
            "body_preview": getattr(msg, "body_preview", "")[:200]
            if getattr(msg, "body_preview", "")
            else "",
        }

    async def _find_folder_by_name(
        self, mailbox_address: str, folder_name: str
    ...
    async def get_inbox_subfolders(self) -> List[Dict[str, Any]]:
        ...
        try:
            logger.info(
                f"Listing subfolders under Inbox for {self.credential_obj.username}"
            )

            all_folders = await self.list_shared_mailbox_folders(
                self.credential_obj.username
            )
            ...
            logger.info(
                f"Found {len(inbox_folder['subfolders'])} subfolders under Inbox"
            )
    ...
    async def _list_subfolders_recursive(
        self,
        mailbox_address: str,
        ...
        try:
            if parent_folder_id is None:
                ...
                if self._is_personal_mailbox(mailbox_address):
                    ...
                    folders = await self.graph_client.users.by_user_id(
                        mailbox_address
                    ).mail_folders.get()
            else:
                # Get subfolders of a specific folder
                if self._is_personal_mailbox(mailbox_address):
                    folders = await self.graph_client.me.mail_folders.by_mail_folder_id(
                        parent_folder_id
                    ).child_folders.get()
                else:
                    folders = (
                        await self.graph_client.users.by_user_id(mailbox_address)
            ...
            logger.error(
                f"Error listing subfolders at depth {depth} for {mailbox_address}: {e}"
            )
            raise
    ...
    async def list_shared_mailbox_folders(
        ...
        try:
            logger.info(f"Listing folders for mailbox: {mailbox_address}")

            folder_list = await self._list_subfolders_recursive(
                mailbox_address=mailbox_address, parent_folder_id=None
            )
    ...
    async def get_shared_mailbox_messages(
        ...
        try:
            ...
            if folder_name.lower() in ["inbox", "indbakke"]:
                ...
                if self._is_personal_mailbox(mailbox_address):
                    messages = (
                        await self.graph_client.me.mail_folders.by_mail_folder_id(
                            "inbox"
                ...
                target_folder = await self._find_folder_by_name(
                    mailbox_address, folder_name
                )
                if target_folder:
                    ...
                    if self._is_personal_mailbox(mailbox_address):
                        ...
                        messages = (
                            await self.graph_client.users.by_user_id(mailbox_address)
                            .mail_folders.by_mail_folder_id(target_folder["id"])
                            .messages.get(request_configuration=request_config)
                        )
                else:
                    logger.warning(
    ...
    async def list_attachments(
        ...
        try:
            ...
            attachments = (
                await self._get_messages_request_builder(mailbox_address)
            ...
            if attachments and attachments.value:
                for attachment in attachments.value:
                    try:
                        ...
                        if attachment_detail and hasattr(
    ...
    async def mark_message_as_read(self, mailbox_address: str, message_id: str) -> bool:
        ...
        try:
            logger.debug(f"Marking message {message_id} as read in {mailbox_address}")
    ...
    async def get_message_body(
        ...
        try:
            ...
            if message and message.body:
                return {
                    "content_type": message.body.content_type.value
    ...
    async def move_message(
        ...
        try:
            ...
            logger.info(
                f"Successfully moved message {message_id} to folder {destination_folder_id}"
            )
            return True
